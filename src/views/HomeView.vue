    ...
    <div class="mx-auto grid grid-cols-4 gap-5 place-items-end h-56 mb-4 p-5">
      <template v-for="(movie, index) in movies.results" :key="movie.id">
        <template v-if="index <=3">
          ...
          :imgSrc="getImage(movie.original_title, movie.poster_path)"
          @toggleBookmark="bookmark.addMovie(movie.original_title, getImage(movie.original_title, movie.poster_path))" 
